...
describe('the well stands where the owner says it stands', () => {
  it('is beside the doorway of the passage that leaves its own storey', () => {
     ...
     *
     * THIS IS THE TEST THAT MOVES WITH THE STAIR. The doorway is re-planned here
     * from the live flight plan by the same call stairDoorways() makes, so the
     * day STAIR_FROM_BUTTRESS_DEG is corrected — and it is known to be at least
     * 8° too small — 171 stops being the tangent and this fails, instead of
     * leaving the well where the old stair bearing happened to put it.
     */
    expect(WALL_LIFTS[DEPARTURE].fromFloorNumber).toBe(3)
    expect(DEPARTURE_DOORWAY.azimuthDeg).toBeCloseTo(190.772, 3)
    ...
    expect(tangent).toBeCloseTo(170.524, 3)
  ...
  it('leaves the mouth 0.87 m from the jamb it stands against', () => {
    ...
    expect(d - WELL.mouthDiameter / 2).toBeCloseTo(0.866, 3)
  })
})
...
describe('what his placement costs, measured rather than argued', () => {
  it('costs no doorway — the fault of 2026-08 is not reopened', () => {
    ...
    expect(worst.gap).toBeGreaterThan(6.7)
  })
  ...
  it('costs no window reveal, which the other side of the door would have', () => {
     ...
     * the tower is 21.5° clear. Clockwise — 211, the mirror tangent — the chase
     * lands in SIX foot reveals in plan and in foot-3-4 in height as well: a pipe
     * out of a window, which is the fault 4f3e197 was written to prevent.
     *
     * That is not why the side was chosen and must never become why. It is a
     * second, independent reason to ask him rather than to assume.
     */
    ...
    expect(Math.min(...revealGap(WELL.azimuthDeg).map((g) => g.gap))).toBeGreaterThan(21.5)
  ...
  it('costs the stair, on every storey the chase runs up', () => {
     ...
     * already inside. Eight lengths of it land in a passage. This asserts the
    ...
    expect(deg[0]).toBeCloseTo(22.61, 2)
    expect(deg[deg.length - 1]).toBeCloseTo(2.3, 1)
    // every storey the chase runs up, not merely some of them
    expect(new Set(breaches.map((b) => b.floorIndex))).toEqual(new Set([2, 3, 4, 5, 6]))
  })
  ...
  it('takes the whole jamb between the room and the stair where it does', () => {
    ...
    expect(Math.min(...breaches.map((b) => b.biteMetres))).toBeCloseTo(0.325, 3)
    expect(Math.max(...breaches.map((b) => b.biteMetres))).toBeCloseTo(0.426, 3)
  })
  ...
  it('opens onto walkable treads on storey 3, which is the worst of it', () => {
     ...
     * opens onto the treads of flight 3→4 themselves, standing between 0.41 and
     * 1.44 m above the floor of the room the walker is in.
     */
    ...
    expect(Math.min(...ys) - chase.bottomY).toBeCloseTo(0.41, 2)
    expect(Math.max(...ys) - chase.bottomY).toBeCloseTo(1.44, 2)
    // and they are inside the chase's own height, so the slot really reaches them
    expect(Math.min(...ys)).toBeGreaterThan(chase.bottomY)
...
describe('where the chase could have stood, kept because it is what was given up', () => {
  ...
  it('leaves exactly one free arc, and the stair closes both its ends', () => {
     ...
     * everything, 160° wide, and the wellhead is 141° away from the middle of it.
     */
    ...
    expect(sep(WELL.azimuthDeg, arc.middleDeg)).toBeCloseTo(141.43, 1)
  ...
  it('has a nearer end than 312, and the near end is 32° from the doorway', () => {
    /*
     ...
     * stops at 21.79. Drop it and the free arc opens at 222.27 instead of 232.14.
     * That bearing is 31.5° CLOCKWISE of the storey-3 doorway, 1.22 m of arc from
     * its far jamb along the room face: still "next to the passage" by any
     * ordinary reading of the words, and it costs nothing at all.
     *
    ...
    expect(relaxed[0].fromDeg - DEPARTURE_DOORWAY.azimuthDeg).toBeCloseTo(31.5, 1)
    const jambToChase =
      ((relaxed[0].fromDeg -
        CHASE_HALF_MAX -
        ...
        Math.PI) /
      180
    expect(jambToChase * innerRadiusAt(FLOORS[WELL.startsAtFloorIndex].floorY)).toBeCloseTo(1.22, 2)
  })
})
